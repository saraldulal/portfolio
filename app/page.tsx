...
export default function Portfolio() {
  ...
  const numExperiencePages = Math.ceil(
    portfolioData.experience.items.length / 2,
  );
  ...
  for (let i = 0; i < numExperiencePages; i++)
  ...
  return (
    <main ref={targetRef} className="relative bg-[#050505] text-[#e0e0e0]">
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        <motion.div
          style={{ x, width: `${totalScreens * 100}vw` }}
        ...
        >
                ...
                <div className="space-y-6 text-[#a0a0a0] text-lg font-light leading-relaxed">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6 border-t border-[#e0e0e0]/10 mt-8">
                    {portfolioData.about.details.map((detail, index) => (
                      <div key={index}>
                        <span className="block text-xs uppercase tracking-widest text-[#e0e0e0] mb-2">
                          {detail.label}
                        </span>
          ...
          <section className="w-screen h-screen shrink-0 overflow-hidden relative">
            <motion.div
              style={{ y: projectsY, height: `${numProjects * 100}vh` }}
              className="flex flex-col w-full"
            >
              {portfolioData.selectedWorks.projects.map((project, index) => (
                <div
                  key={index}
                  className="w-screen h-screen flex items-center px-8 md:px-24 shrink-0"
                >
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 w-full h-full py-24">
                    ...
                    >
                      <span className="text-sm tracking-widest uppercase text-[#a0a0a0] mb-4">
                        ...
                        {portfolioData.selectedWorks.title}
                      </span>
                      ...
                      <a
                        href={project.link}
                        className="inline-flex items-center gap-2 text-sm tracking-widest uppercase hover:text-white transition-colors w-fit group"
                      >
                        View Project{" "}
          ...
          <section className="w-screen h-screen shrink-0 overflow-hidden relative">
            <motion.div
              style={{
                y: experienceY,
                height: `${numExperiencePages * 100}vh`,
              }}
              className="flex flex-col w-full"
            >
              {Array.from({ length: numExperiencePages }).map(
                (_, pageIndex) => {
                  const itemsForPage = portfolioData.experience.items.slice(
                    pageIndex * 2,
                    pageIndex * 2 + 2,
                  );
                      ...
                      <div className="w-full max-w-4xl mx-auto">
                        ...
                        <div className="space-y-12">
                          {itemsForPage.map((item, itemIndex) => (
                            ...
                            >
                              <div className="text-[#a0a0a0] text-sm tracking-widest uppercase mt-1">
                                {item.time}
                              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 w-full max-w-5xl items-center">
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-sm tracking-widest uppercase text-[#a0a0a0]">
                    Email
                  </span>
                  <a
              ...
              >
                <div className="flex flex-col gap-2">
                  <label
                    htmlFor="name"
                    className="text-sm tracking-widest uppercase text-[#a0a0a0]"
                  >
                    Name
                  </label>
                  <input
